...
Spreadsheet.prototype.send = function(callback, options) {
    ...
    function autoSize(next){
      if(options.autoSize){
        _this.getMetadata(function(err, metadata){
          ...
          var rows = metadata.get('rowCount');
          var cols = metadata.get('colCount');
          if(rows<_this.maxRow){
            metadata.set('rowCount', _this.maxRow);
          }
          if(cols<_this.maxCol){
            metadata.set('colCount', _this.maxCol);
          }
...
Spreadsheet.prototype.getMetadata = function(callback){
  var _this = this;
  if(this.metadata){ return callback(null, this.metadata); }
  request({
    method: 'GET',
    url: this.protocol+'://spreadsheets.google.com/feeds/worksheets/' + this.spreadsheetId+'/private/full/'+this.worksheetId+'?alt=json',
    headers: this.authHeaders
  }, function(err, response, body) {
    if(err){ return callback(err); }
    var result = JSON.parse(body);
    _this.metadata = new Metadata(_this, result);
