...
exports.create = function(opts) {
  ...
  _.extend(spreadsheet, _.pick( opts,
    ...
    'worksheetId', 'worksheetName', 'debug', "meta"
  ));
...
Spreadsheet.prototype.getSheetId = function(type, callback) {
  ...
  request({
    ...
    if(entries.length > 0 && _.isEmpty(_this.meta)) {
      _this.meta.worksheets = [];

      for(var en in entries) {

        var ws = {};
        ws.title = entries[en].title.$t;
        ws.row_count = entries[en].gs$rowCount.$t;
        ws.column_count = entries[en].gs$colCount.$t;

        _this.meta.worksheets.push(ws);
      }
      
      _this.meta.worksheet_count = _this.meta.worksheets.length;
    }
